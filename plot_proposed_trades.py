...
def plot_allocation(ax: plt.Axes, df: pd.DataFrame, title: str, *, bucket4: bool = False) -> None:
    ...
    if bucket4:
        ...
        sort_col = "_plot_long"
        legend_long = mpatches.Patch(color="steelblue", alpha=0.85, label="Underlying hedge (short)")
        legend_short = mpatches.Patch(color="tomato", alpha=0.85, label="Inverse ETF (short)")
    else:
        df = df.assign(
        ...
        sort_col = "long_usd"
        legend_long = mpatches.Patch(color="steelblue", alpha=0.85, label="Underlying (long)")
        legend_short = mpatches.Patch(color="tomato", alpha=0.85, label="ETF (short)")
    df = df.sort_values(sort_col, ascending=True).reset_index(drop=True)
    y = np.arange(len(df))
    xmax = max(df["_plot_long"].abs().max(), df["_plot_short"].abs().max(), 1)
    pad = xmax * 0.012
    ...
    for i, row in df.iterrows():
        ...
        if bucket4:
            ...
            if lu_abs > 1:
                ax.text(lu + pad, i, str(row.get("Underlying", "")),
                        ha="left", va="center", fontsize=6.5, color="steelblue")
            if su_abs > 1:
                ax.text(su - pad, i, str(row.get("ETF", "")),
                        ha="right", va="center", fontsize=6.5, color="tomato")
        else:
            if abs(row["long_usd"]) > 1:
                ax.text(row["_plot_long"] + pad, i, str(row.get("Underlying", "")),
                        ha="left", va="center", fontsize=6.5, color="steelblue")
            if abs(row["short_usd"]) > 1:
                ax.text(row["_plot_short"] - pad, i, str(row.get("ETF", "")),
                        ha="right", va="center", fontsize=6.5, color="tomato")
    ...
    ax.legend(handles=[legend_long, legend_short], fontsize=8, loc="lower right")
