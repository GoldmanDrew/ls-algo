...
that ``delta_estimator.DeltaPrior.for_row`` uses for ``covered_call_1x``
...
The script:
  ...
  2. Computes a robust EWMA hedge-delta vs the underlying using the same
     :func:`delta_estimator.compute_delta_for_hedging` routine but with
     a deliberately weak prior (``tau = 0``) so the output is data-only.
  3. Reports the median, the inter-quartile range, and writes
     ``data/delta_priors_calibration.json``.
...
explicitly (commit the new constant in ``delta_estimator`` with a
...
from delta_estimator import (  # noqa: E402  — path setup above
    DeltaPrior,
...
def main() -> int:
    ...
    ap.add_argument(
        "--out",
        default=str(ROOT / "data" / "delta_priors_calibration.json"),
    ...
    for etf, und in _PAIRS:
        ...
        res = compute_delta_for_hedging(s_etf, s_und, prior)
        rows.append(
            {
                "etf": etf,
                "underlying": und,
                "beta": res.delta,
                "delta_se": res.delta_se,
                "n_obs": res.n_obs,
                "horizon": res.horizon,
                "quality": res.quality,
            }
        ...
        print(
            f"[CC-CAL] {etf:6s} vs {und:5s}  β={res.delta:+.3f}  "
            f"se={res.delta_se:.3f}  n={res.n_obs}  q={res.quality}"
        )
    ...
    if 0.45 <= median <= 0.65:
        print(
            "[CC-CAL] Within tolerance: keep cc_default_mu = 0.55 in delta_estimator."
        )
    else:
        print(
            "[CC-CAL] Out of tolerance: update cc_default_mu in delta_estimator "
